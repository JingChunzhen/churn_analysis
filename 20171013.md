...
#### 实验流程
本实验中，使用狂暴之翼在2016.10.10 0:0:47至201.10.13 0:0:0的埋点数据，以自然天进行分割，得到第一，二， 三天的数据，在这三天的数据中，分别获得第一天，第二天，第三天玩家的特征和标签

在第一天的数据中，在流失玩家中需要剔除操作动作数量较少的玩家，绘制动作操作数和流失人数的曲线如下图

<center>![](/home/jingchunzhen/projects/churn_analysis/fc100churn.png)</center>

上图中，横坐标表示的是动作的操作数量，纵坐标表示的是人数，在流失玩家曲线上，从横坐标0到16曲线斜率很大，表明游戏初期玩家流失较多，这一部分的玩家很可能是由于对游戏的题材不感兴趣而流失，这部分的玩家对游戏的分析无太大意义，故剔除操作动作数量小于16的玩家，对于第二和第三天的流失玩家，不剔除操作动作数量较少的玩家，因为这一部分的玩家已经不太可能因为对游戏的题材不感兴趣而流失
在第一天的玩家中，剔除一部分操作数量较少的玩家之后，大约有60%会流失，故对非流失玩家进行采样，得到的流失玩家和非流失玩家之间的比例为6348:6142，比例大约为1:1，第二天游戏玩家大约有50%会流失，所以未进行采样，在第三天中，由于数据本身的原因（第四天的数据中只统计了很小的时间段内），未进行分析，实验流程图如下
...
* 用户是否是vip用户，由于只有6%的VIP用户会流失，所以用户是否是vip用户也反映了用户对于游戏的喜爱程度
 ...
 单独使用上述特征得到的准确率为65%，使用XGBoost模型在第一自然天的数据中提取上述特征中较为重要的特征，所提取出来的较为重要的特征是玩家在该自然天的游戏总时长，动作种类，体力的变化次数和最大体力，金币的变化次数和最大金币量，最大战力，等级的平均增长速度，动作间的平均时间间隔，而玩家在该自然天所得的黑钻，钻石相关的特征则无明显关联；
 
 分析上述重要特征，玩家等级的平均增长速度和动作间平均时间间隔反映了玩家对于游戏的熟悉程度和专心程度，也由此反映了玩家对于游戏的喜爱程度，也有可能是玩家是从别的服务器转来的，故对游戏比较熟悉
 
 最大时间间隔对玩家的流失与否无明显关联，可能是因为最大时间间隔本身具有很大的偶然性
 
 用户的vip属性对流失预测无较大影响，可能是因为在整体的玩家中，vip用户本身占比较少
 
玩家的游戏时长，玩家所操作的动作种类， 玩家金币的变化次数和最大金币量以及体力的变化次数和最大体力影响很大，猜测可能是因为这几个特征都和时间有很大的关系，如玩家玩的时间越长体力的变化次数和最大的体力自然越大，而玩家玩的时间越长越不易流失（此处应该有统计数据证明），所以需要对上述特征做进一步的处理，具体的做法是计算上述特征与玩家在改天玩的游戏总时长的比值，抛开玩家的游戏总时长和动作的种类这些与玩家游戏时间有关的特征的影响重新抽取重要特征，得到重要的特征为玩家体力的平均增长速度，玩家金币在单位时间内的变化，玩家等级在单位时间内的变化（此处应该有统计数据证明）

（这里统计流失玩家和非流失玩家在金币获取速度上和体力变化上的均值和中位数）
（这里需要对第二天的数据进行重新统计）
